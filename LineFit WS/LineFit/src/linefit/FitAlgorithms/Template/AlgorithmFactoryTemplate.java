/*
Copyright (C) *year*  *name or organization* //TODO:fill in this information for the copyright licensing
...
package linefit.FitAlgorithms.Template; //TODO: Remove the ".template" part and move the created class to the higher package, FitAlgorithms
...
//TODO: after you are finished with this coding, make sure you add your algorithmFactory to the appropriate array in LinearFitFactory or else it wont show up in LineFit!

//TODO: fill in the class information
/**
 * The *algorithm name* Factory class that is used to create the *algorithm name* Strategy so that it can be done in a generic way during run time, allowing the user to change the algorithm
 * Feel free to explain high level aspects of your algorithm here
 * @author Your Name
 * @version	1.0
 * @since "current LineFit version number"
 */
class AlgorithmFactoryTemplate extends LinearFitFactory //TODO: Rename this using the format *algorithm name*Factory, i.e. if it was named Chi Squared Fit, it would be ChiSquaredFitFactory
{		
	...
	/** The constructor that allows us to create the *name* Fit Data without knowing which factory it is and also determines whether or not its child can fix its slope or intercept */
	...
	/**
	 * The private class for the *algorithm name* Algorithm that does the calculations and that is made by the *algorithm name* Fit Algorithm
	 * @author your name
	 * @version	1.0
	 * @since *current LineFit version Number*
	 */
	private class AlgorithmStrategyTemplate extends LinearFitStrategy //TODO:Rename this using the format *algorithm name*Strategy, i.e. if it was named Chi Squared Fit, it would be ChiSquaredFitStrategy
	{		
		...
		//TODO: This is where your algorithm is actually implemented - you can use as many private helper functions as needed
		/**
		...
		{
			//algorithm stuff
			/* the ChiSquare fit for one error is the correct way to graph for one error so this should look like this in most circumstances
			if(fitTypeToUse == FitType.BOTH_ERRORS)
			{
				//your fit algorithm for fitting both errors
			}
			else
			{
				defaultChiSquareFitForSingleError(fitTypeToUse);
			}
			*/
		}
