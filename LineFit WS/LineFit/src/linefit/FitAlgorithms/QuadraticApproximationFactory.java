/*
Copyright (C) 2015  Covenant College Physics Department
...
/**
 * The Quadratic Approximation Factory class that is used to create the Quadratic Approximation Strategy so that it can be done in a generic way during run time, allowing the user to change the algorithm
 * This algorithm makes use of the quadratic equation to calculate the Chi Squared values for the data with given slopes and intercepts
 * Chi Squared (S) is written as: S1 = S0 + alpha(b1 - b0)^2 + beta(m1 - m0)^2 + 2gamma(b1 - b0)(m1 - m0), where S1 is the Chi squared
 * being calculated with the given m1 and b1 and S0 is the Chi Square of the calculated starting point and m0 and b0 are the calculated
...
{		
	...
	/** The constructor that allows us to create the Quadratic Approximation Algorithm Fit Strategy without knowing which factory it is and also determines whether or not its child can fix its slope or intercept */
	...
	/** Creates a new instance of the type of FitAlogritm this Factory is
	...
	/**
	 * The private class for the Quadratic Approximation Algorithm that does the calculations and that is made by the Quadratic Approximation Fit Algorithm
	 * @author Don Petcher and Keith Rice
	 * @version	1.0
	 * @since 0.98.1
	 */
	...
	{		
		/** the linear Fit Data we use as a starting point to find the intitial slopes and intercept and errors to use */
		...
		private QuadraticApproximationStrategy(DataSet dataSet) 
		{
			...
			//create the this linear fit strategy to serve as an initial estimate for the Quadratic Minimization function
		...
		/** calculates the error and fit for a line with both errors by using quadratic minimization and the use of the ChiSquared (S in the equations) */
		private void approximateQuadratically()
		{
			...
			double gamma = (delta61 - delta31 - (0.5 * (1 + epsilon) * delta51) - (0.5 * (1 - epsilon) * delta41)) / (2 * epsilon * sigmaB * sigmaM);
...
//			}
			...
			double b0 = startingIntercept + oneOverABMinusGSquared * (beta * D41MinusD51Over4Sb - gamma * D21MinusD31Over4Sm);
			double m0 = startingSlope + oneOverABMinusGSquared * (alpha * D21MinusD31Over4Sm - gamma * D41MinusD51Over4Sb);
			double chiSquared0 = chiSquared1 - (alpha * Math.pow(startingIntercept - b0, 2) + beta * Math.pow(startingSlope - m0, 2) +
					2 * gamma * (startingIntercept - b0) * (startingSlope - m0));
